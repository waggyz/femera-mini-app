...
#if 0
  __m256d c0,c1,c2;
  c0 = _mm256_set_pd(0.,C[5],C[3],C[0]); c1 = _mm256_set_pd(0.,C[4],C[1],C[3]); c2 = _mm256_set_pd(0.,C[2],C[4],C[5]);
#endif
...
// Slower without this variable even though it isn't used (44->42) UNTIL I added the aligned decorations above!  Indicates other padding may help.
  ...
  for(int ip=0; ip<intp_n; ip++){
    ...
    _MM256_TRANSPOSE3_PD(h036,h147,h258);
...
#if 0
// Slightly slower than the scalar version below (440 MDOF -> 433)
    __m256d s048;
    s048= _mm256_mul_pd(_mm256_set1_pd(dw),_mm256_add_pd(_mm256_mul_pd(c0,_mm256_set1_pd(H[0])), _mm256_add_pd(_mm256_mul_pd(c1,_mm256_set1_pd(H[4])), _mm256_mul_pd(c2,_mm256_set1_pd(H[8])))));
          _mm256_mask_i32scatter_pd(&S[0], 0x7, _mm_set_epi32(0,64,32,0), s048, 1);
    S[1]=(H[1] + H[3])*C[6]*dw;
    S[2]=(H[2] + H[6])*C[8]*dw;
    S[5]=(H[5] + H[7])*C[7]*dw;
#else
