...
int ElastIso3D::ElemLinear( Elem* E,
...
#endif
  //INT_MESH conn[Nc];
  //INT_MESH this_conn[Nc],next_conn[Nc];
  ...
  //        INT_MESH*  thisc = &this_conn[0];
  //        INT_MESH*  nextc = &next_conn[0];
  //
  if(e0<ee){
    ...
    for (int i=0; i<Nc; i++){
      std::memcpy( &    u[Nf*i],
                   &sysu[E->elem_conn[Nc*e0+i]*Nf], sizeof(FLOAT_SOLV)*Nf ); };
    //if((e0+1)<ee){
  ...
  for(INT_MESH ie=e0;ie<ee;ie++){
    ...
    //std::memcpy( &jac , &E->elem_jacs[Nj*ie], sizeof(FLOAT_MESH)*Nj);
    ...
    for (int i=0; i<Nc; i++){
      std::memcpy( & f[Nf*i],
        & sysf[E->elem_conn[Nc*ie+i]*Nf], sizeof(FLOAT_SOLV)*Nf ); 
      //std::memcpy( & u[Nf*i],
      //  & sys_u[E->elem_conn[Nc*ie +i]*Nf], sizeof(FLOAT_SOLV)*Nf ); 
    };
    //const FLOAT_PHYS J = Ejacs[Nj*ie+9];
    const FLOAT_PHYS J = jac[9];
