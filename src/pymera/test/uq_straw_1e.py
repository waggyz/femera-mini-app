...
def main():
    ...
    sims = jobs.add_sims_from_file(os.path.join(here, 'uq_straw_1e.json'))
    #
    ...
    sims.add_parameter_file(os.path.join(here, 'uq_straw_1e.csv'),
    ...
    #
    flat = flatten_json_file(os.path.join(here, 'uq_straw_1e.json'))
    print()
    ...
    if False:# Write random input parameters to a .csv file.
        ...
        with open('src/pymera/test/uq_straw_1e.csv', "w") as f:
            writer = csv.writer(f)
