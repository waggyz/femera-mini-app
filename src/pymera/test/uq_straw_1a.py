...
# Set up random variables as size N numpy arrays.
tip_displacement_z = np.random.normal(0.100, 0.010, N) # mean=0.100, stdev=0.010
length = np.random.normal(1.000, 0.010, N)
width = np.random.normal(0.050, 0.005, N)
height = np.random.normal(0.050, 0.005, N)
...
sims.set_parameter(name='fmr:geom:length', to=length) #x (global parameter)
